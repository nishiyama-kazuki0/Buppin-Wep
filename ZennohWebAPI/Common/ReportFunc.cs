...
namespace ZennohWebAPI.Common
{
    public class ReportFunc
    {
        ...
        /// 入荷票再出力
        /// </summary>
        public static bool 入荷票再出力(string DEVICE_ID, string USER_ID, string BASE_ID, int BASE_TYPE, string CONSIGNOR_ID, string 受付No, string 入荷No)
        {
            return Proc入荷票出力(DEVICE_ID, USER_ID, BASE_ID, BASE_TYPE, CONSIGNOR_ID, 受付No, true, 入荷No);
        }
        ...
        /// <param name="dt"></param>
        private static bool Proc入荷票出力(string DEVICE_ID, string USER_ID, string BASE_ID, int BASE_TYPE, string CONSIGNOR_ID, string 受付No, bool 再発行, string? 入荷No = null)
        {
            LogTo.Information($"DEVICE_ID:{DEVICE_ID},USER_ID:{USER_ID},BASE_ID:{BASE_ID},BASE_TYPE:{BASE_TYPE},CONSIGNOR_ID:{CONSIGNOR_ID},受付No:{受付No},再発行:{再発行},入荷No:{入荷No ?? string.Empty}");
            try
            {
                // プリンター名を取得
                ...
                {
                    select.whereParam.Add("入荷No", new WhereParam { val = 入荷No });
                }
        ...
        private static bool Proc在庫一覧リスト出力(string DEVICE_ID, string USER_ID, string BASE_ID, int BASE_TYPE, string CONSIGNOR_ID, string 入庫日From, string 入庫日To, string 入荷No, string 明細No, string 品名コード, string 課コード, string 出荷者コード, string 産地コード, string パレットNo, string 賞味期限From, string 賞味期限To, string 倉庫コード, string ゾーンコード, string ロケーションNo, string 滞留期間, int 滞留期間whereType, string 在庫有無, string 資材管理, bool 再発行)
        {
            ...
            try
            {
                ...
                {
                    if (!Enum.TryParse<enumWhereType>(滞留期間whereType.ToString(), out enumWhereType type))
                    {
