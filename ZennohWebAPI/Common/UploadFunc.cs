...
namespace ZennohWebAPI.Common
{
    public class UploadFunc
    {
        ...
        /// ファイル取込
        /// </summary>
        public static (int retCode, string retMsg) ファイル取込(string DEVICE_ID, string USER_ID, string BASE_ID, int BASE_TYPE, string CONSIGNOR_ID, string ProgramName, string FilePath, string ClassName)
        {
            (int retCode, string retMsg) ret = (0, string.Empty);
            try
            {
                LogTo.Information($"DEVICE_ID:{DEVICE_ID},USER_ID:{USER_ID},BASE_ID:{BASE_ID},BASE_TYPE:{BASE_TYPE},CONSIGNOR_ID:{CONSIGNOR_ID},ProgramName:{ProgramName},FilePath:{FilePath},ClassName:{ClassName}");
                ...
                _ = requestValue.SetArgumentValue(SharedConst.KEY_BASE_ID, BASE_ID, string.Empty);
                _ = requestValue.SetArgumentValue(SharedConst.KEY_BASE_TYPE, BASE_TYPE, string.Empty);
                _ = requestValue.SetArgumentValue(SharedConst.KEY_CONSIGNOR_ID, CONSIGNOR_ID, string.Empty);
        ...
        private static (int retCode, string retMsg) CheckImportFileDataFormat(ProcessFunctionModel f, RequestValue r, SqlConnection con, Encoding fenc)
        {
            ...
            IEnumerable<UserTableTypeColumnModel> userTableTypeColumnList = DataSource.GetEntityCollection<UserTableTypeColumnModel>(
                    con
            ...
            foreach (KeyValuePair<int, List<ArgumentValue>> reqRow in r.ArgumentDataSet)
            {
                ...
                foreach (UserTableTypeColumnModel utColumn in userTableTypeColumnList)
                {
                    ...
                    bool isUseMaxLength = isSizeSpecNeeded(DataSource.GetDbType(utColumn.DataType));
        ...
        //TODO 他クラスにも類似の処理を宣言しているので、可能であればまとめる。が用途が異なるため、難しいかもしれない。暫定
        //TODO むしろ数値の場合以外はtrueとして、trueの場合はすべてMaxLengthを使用してチェックしたほうが安全か。要検討。
        /// <summary>
        /// DBTypeが文字列かどうかを判定する
        /// 文字列の場合はMaxLengthを使用するかどうかの判断に使用する
        /// </summary>
        /// <param name="type"></param>
        /// <returns>文字列の場合true</returns>
        private static bool isSizeSpecNeeded(SqlDbType type)
        {
            return type is SqlDbType.Char
