...
namespace ZennohBlazorShared.Pages
{
    ...
    /// 在庫移動実績管理 作業者別
    /// </summary>
    public partial class TabItemProductivityStocksPersons : TabItemBase
    {
        #region override
        ...
        public override async Task RefreshGridData(Dictionary<string, WhereParam> whereParam, string strViewName = "", string attributeName = STR_ATTRIBUTE_GRID, bool bInitSelect = false)
        {
            try
            {
                ...
                strArg += whereParam.ContainsKey("移動元倉庫コード") ? ", @移動元倉庫コード" : ", (null)";
                strArg += whereParam.ContainsKey("移動元ゾーンコード") ? ", @移動元ゾーンコード" : ", (null)";
                strArg += whereParam.ContainsKey("移動先倉庫コード") ? ", @移動先倉庫コード" : ", (null)";
                strArg += whereParam.ContainsKey("移動先ゾーンコード") ? ", @移動先ゾーンコード" : ", (null)";
