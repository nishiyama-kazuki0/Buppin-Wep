...
namespace ZennohBlazorShared.Pages
{
    ...
    /// 出荷進捗照会 倉庫別
    /// </summary>
    public partial class TabItemShipmentsProgressAreas : TabItemBase
    {
        public const string STR_ATTRIBUTE_GRID_ALL = "AttributesGridAll";
        public const string STR_GRID_VIEW_NAME = "VW_出荷進捗_倉庫別";
        public const string STR_GRID_ALL_VIEW_NAME = "VW_出荷進捗_倉庫別_全体進捗";
        ...
        public const string STR_GRID_COL_コーナー別仕分済数 = "コーナー別\n仕分済数";
        public const string STR_GRID_COL_コーナー別仕分進捗率 = "コーナー別\n仕分進捗率";
        public const string STR_GRID_COL_PRG_コーナー別仕分進捗率 = DataGridProgress.KEY_PROGRESS + "コーナー別仕分進捗率";
        ...
        protected override async Task InitAttributesAsync()
        {
            ...
            for (int i = 0; _componentsInfo.Count > i; i++)
            {
                ...
                catch (Exception ex) { _ = ComService.PostLogAsync(ex.Message); }
            }
        }
            ...
            try
            {
                ...
                if (_gridData != null && _gridData.Count() > 0)
                {
                    ...
                    if (_gridAllData != null && _gridAllData.Count() > 0)
                    {
                        ...
                        _gridAllData[0][STR_GRID_COL_PRG_ピック進捗率] = _gridAllData[0][STR_GRID_COL_ピック進捗率] = ComService!.GetPercent(decピック済数, dec引当済数, 1);
                        _gridAllData[0][STR_GRID_COL_コーナー別仕分済数] = decコーナー別仕分済数;
                        _gridAllData[0][STR_GRID_COL_PRG_コーナー別仕分進捗率] = _gridAllData[0][STR_GRID_COL_コーナー別仕分進捗率] = ComService!.GetPercent(decコーナー別仕分済数, dec引当済数, 1);
                        _gridAllData[0][STR_GRID_COL_コーナー搬送済数] = decコーナー搬送済数;
                        _gridAllData[0][STR_GRID_COL_PRG_搬送進捗率] = _gridAllData[0][STR_GRID_COL_搬送進捗率] = ComService!.GetPercent(decコーナー搬送済数, dec引当済数, 1);
                    }
                }
