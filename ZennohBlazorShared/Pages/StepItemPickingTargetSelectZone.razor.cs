...
namespace ZennohBlazorShared.Pages
{
    ...
    /// パレットピッキング【倉庫別】/ゾーン選択
    /// </summary>
    public partial class StepItemPickingTargetSelectZone : StepItemBase
    {
        ...
        public override Task 確定前処理(ComponentProgramInfo info)
        {
            ...
            {
                if (_gridSelectedData[0].TryGetValue("ｿﾞｰﾝｺｰﾄﾞ", out object? obj))
                {
                    model!.ZoneCd = (string)(obj ?? "");
                }
                if (_gridSelectedData[0].TryGetValue("ｿﾞｰﾝ名", out obj))
                {
                    model!.ZoneNm = (string)(obj ?? "");
                }
            }
