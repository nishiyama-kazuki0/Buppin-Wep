...
{
    /// <summary>
    /// TextBoxZeroSuppressコンポーネント
    /// </summary>
    public partial class CompTextBoxZeroSuppress : CompBase
    {
        /// <summary>
        /// 初期値
        /// </summary>
        ...
        {
            // 初期値設定
            //InputValue = InitialValue.TrimStart('0');
            InputValue = int.TryParse(InitialValue, out int r) ? $"{r:0000}" : InitialValue;
        ...
        public override void AddWhereParam(Dictionary<string, (object, WhereParam)> retWhereParam, string keyNmae)
        {
            ...
            {
                int len = MaxLength == null ? 0 : (int)MaxLength;
                retWhereParam[keyNmae] = len > 0
                    //? (InputValue.PadLeft(len, '0'), new WhereParam { val = InputValue, whereType = enumWhereType.LikeStartZeroSuppress })
                    ? (int.TryParse(InitialValue, out int r) ? $"{r:0000}" : InitialValue, new WhereParam { val = InputValue, whereType = enumWhereType.LikeStartZeroSuppress })
                    : ((object, WhereParam))(InputValue, new WhereParam { val = InputValue, whereType = enumWhereType.LikeStartZeroSuppress });
            }
        }
