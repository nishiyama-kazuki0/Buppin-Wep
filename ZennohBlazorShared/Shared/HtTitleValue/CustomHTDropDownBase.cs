...
namespace ZennohBlazorShared.Shared.HtTitleValue
{
    /// <summary>
    /// Base class of components that display a list of items.
    /// </summary>
    /// <typeparam name="T"></typeparam>
    public class CustomHTDropDownBase<T> : CustomHTDataBoundFormComponent<T>
    {
        [Inject]
        protected NotificationService NotificationService { get; set; }
        [Inject]
        protected CommonService ComService { get; set; } = null!;
        public bool FocusOutFlag = false;
        public bool KeyPressProcFlag = false;
        ...
        /// </summary>
        public Microsoft.AspNetCore.Components.Web.Virtualization.Virtualize<object> Virtualize => virtualize;

        private List<object>? virtualItems;
        ...
        {
            IEnumerable<object> data = Data != null ? Data.Cast<object>() : Enumerable.Empty<object>();
            IQueryable<object> view = (LoadData.HasDelegate ? data : View).Cast<object>().AsQueryable();
            int totalItemsCount = LoadData.HasDelegate ? Count : view.Count();
            int top = request.Count;
        ...
        internal virtual RenderFragment RenderItems()
        {
            return new RenderFragment(builder =>
            {
                ...
                {
                    foreach (object? item in LoadData.HasDelegate ? Data : View)
                    {
            ...
            get
            {
                ...
                else
                {
                    return typeof(IEnumerable).IsAssignableFrom(typeof(T))
                        ? internalValue != null && ((IEnumerable)internalValue).Cast<object>().Any()
                        : internalValue != null;
                }
            }
        }
        ...
        private readonly System.Collections.Generic.HashSet<object> keys = new();

        internal object GetKey(object item)
        {
            object value = GetItemOrValueFromProperty(item, ValueProperty);
        ...
        protected async System.Threading.Tasks.Task SelectAll()
        {
            ...
            {
                selectedItems.Clear();
                selectedItems = View.Cast<object>().ToList().Where(i => disabledPropertyGetter == null || (disabledPropertyGetter(i) as bool?) != true).ToList();
            }
            else
            {
                selectedItems.Clear();
            ...
            {
                Type type = typeof(T).IsGenericType ? typeof(T).GetGenericArguments()[0] : typeof(T);
            ...
            if (typeof(IList).IsAssignableFrom(typeof(T)))
            {
                ...
                foreach (object? i in (IEnumerable)internalValue)
                {
                    _ = list.Add(i);
                }
            ...
            {
                IList? list = (IList)Activator.CreateInstance(typeof(List<>).MakeGenericType(typeof(T).GetGenericArguments()[0]));
                foreach (object? i in (IEnumerable)internalValue)
                {
                    _ = list.Add(i);
                }
        ...
        {
            return LoadData.HasDelegate && !string.IsNullOrEmpty(ValueProperty)
                ? View != null && View.Cast<object>().ToList()
                ...
                : View != null && selectedItems.Count == View.Cast<object>().ToList()
                    .Where(i => disabledPropertyGetter == null || (disabledPropertyGetter(i) as bool?) != true).Count();
        }
        ...
        public void Reset()
        {
            _ = InvokeAsync(ClearAll);
            _ = InvokeAsync(DebounceFilter);
        }
        ...
        protected async System.Threading.Tasks.Task ClearAll()
        {
            ...
            _ = await JSRuntime.InvokeAsync<string>("Radzen.setInputValue", search, "");
        ...
        protected override void OnParametersSet()
        {
            ...
            if (_data != null)
            {
                IQueryable query = _data.AsQueryable();
        ...
        // フォーカス遷移のためidを指定した場合、キャッシュが残るのかpopup-指定IDではポップアップのdisplayが更新されなかった
        // そのためポップアップのIDはユニークIDを付加することで対応
        //protected string PopupID => $"popup-{GetId()}";
        protected string PopupID => $"popup-{UniqueID}";
        ...
        // ポップアップIDと合わせるため変更
        //protected string SearchID => $"search-{GetId()}";
        protected string SearchID => $"search-{UniqueID}|{GetId()}";
        ...
        {
            return Disabled ? string.Empty : $"Radzen.togglePopup(this.parentNode, '{PopupID}', true);Radzen.focusElement('{SearchID}');";
        }
        ...
        {
            return Disabled ? string.Empty : $"Radzen.togglePopup(this, '{PopupID}', true);Radzen.focusElement('{SearchID}');";
        }
        ...
        protected virtual async System.Threading.Tasks.Task<bool> ArrowDownKeyPush()
        {
            await Task.Delay(0);
            return true;
        }
        ...
        private async System.Threading.Tasks.Task HandleKeyPress(Microsoft.AspNetCore.Components.Web.KeyboardEventArgs args, bool isFilter = false)
        {
            ...
            // KeyenceHTのブラウザはargs.Codeは空白になっているためargs.Keyを使用する
            //string key = args.Code ?? args.Key;
            string key = args.Key;
            //_ = ComService.PostLogAsync($"{UniqueID}:HandleKeyPress key:{key} alt:{args.AltKey}");
            ...
            if (!args.AltKey && (key == "ArrowDown" || key == "ArrowUp"))
            {
                try
                {
                    // ArrowDownキーが押された時の処理
                    if (key == "ArrowDown" && false == await ArrowDownKeyPush())
                    {
                        KeyPressProcFlag = false;
                        return;
                    }

                    int currentViewIndex = Multiple ? selectedIndex : items.IndexOf(selectedItem);

                    int newSelectedIndex = await JSRuntime.InvokeAsync<int>("Radzen.focusListItem", search, list, key is "ArrowDown" or "ArrowRight", currentViewIndex);

                    if (!Multiple)
                    {
                        //_ = ComService.PostLogAsync($"{UniqueID}:HandleKeyPress currentViewIndex:{currentViewIndex} newSelectedIndex:{newSelectedIndex} items.Count:{items.Count() - 1}");
            ...
            {
                //_ = ComService.PostLogAsync($"{UniqueID}:HandleKeyPress OpenPopup:{key}");
                await OpenPopup(key, isFilter);
            }
            else if (key is "Escape" or "Tab")
            {
...
#if NET5_0_OR_GREATER
            return AllowVirtualization
                ? await Task.FromResult(new Radzen.LoadDataArgs() { Skip = 0, Top = PageSize, Filter = searchText })
                : await Task.FromResult(new Radzen.LoadDataArgs() { Filter = searchText });
#else
        ...
        public override async Task SetParametersAsync(ParameterView parameters)
        {
...
#endif
            bool selectedItemChanged = parameters.DidParameterChange(nameof(SelectedItem), SelectedItem);
            ...
            {
                bool visible = parameters.GetValueOrDefault<bool>(nameof(Visible));
                shouldClose = !visible;
            }
        ...
        {
            if (internalValue is IEnumerable valueAsEnumerable)
            {
        ...
        internal bool IsSelected(object item)
        {
            ...
            {
                return Multiple ? selectedItems.IndexOf(item) != -1 : object.Equals(item, selectedItem);
            }
        }
        ...
        protected virtual IEnumerable<object> Items => (LoadData.HasDelegate ? Data ?? Enumerable.Empty<object>() : (View ?? Enumerable.Empty<object>())).Cast<object>();
        ...
        protected override IEnumerable View
        {
            get
            {
                if (_view == null && Query != null)
                {
                    ...
                    {
                        bool ignoreCase = FilterCaseSensitivity == FilterCaseSensitivity.CaseInsensitive;
                        ...
                        _view = Query.ElementType == typeof(Enum)
                            ? (IQueryable)Query.Cast<Enum>()
                                .Where((Func<Enum, bool>)(i =>
                                {
                                    string value = ignoreCase ? i.GetDisplayDescription().ToLower() : i.GetDisplayDescription();
                            ...
                            : Query.Where(string.Join(".", query), search);
                    }
                    else
                    {
                        _view = IsVirtualizationAllowed()
                            ? Query
                            : typeof(IQueryable).IsAssignableFrom(Data.GetType()) ? (Query as IEnumerable).Cast<object>().ToList().AsQueryable() : Query;
                    }
                }
        ...
        public async System.Threading.Tasks.Task SelectItem(object item, bool raiseChange = true)
        {
            ...
            if (!Multiple)
            {
                ...
                selectedItem = item;
                internalValue = !string.IsNullOrEmpty(ValueProperty) ? PropertyAccess.GetItemOrValueFromProperty(item, ValueProperty) : item;
            ...
            else
            {
                ...
                else
                {
                    ...
                    internalValue = elementType != null ? selectedItems.AsQueryable().Cast(elementType) : selectedItems;
                }
            }
            if (raiseChange)
            {
                if (ValueChanged.HasDelegate)
                {
                    ...
                    {
                        await ValueChanged.InvokeAsync(object.Equals(internalValue, null) ? default : (T)internalValue);
                    }
                }
        ...
        {
            IEnumerable view = LoadData.HasDelegate ? Data : View;
            if (value != null && view != null)
            {
                if (!Multiple)
                {
                    if (!string.IsNullOrEmpty(ValueProperty))
                    {
                        SelectedItem = typeof(EnumerableQuery).IsAssignableFrom(view.GetType())
                            ? view.OfType<object>().Where(i => object.Equals(GetItemOrValueFromProperty(i, ValueProperty), value)).FirstOrDefault()
                            : (object)view.AsQueryable().Where($@"{ValueProperty} == @0", value).FirstOrDefault();
                    }
                    else
                    {
                ...
                else
                {
                    if (value is IEnumerable values)
                    {
                        if (!string.IsNullOrEmpty(ValueProperty))
                        {
                            ...
                            {
                                dynamic item = typeof(EnumerableQuery).IsAssignableFrom(view.GetType())
                                    ? view.OfType<object>().Where(i => object.Equals(GetItemOrValueFromProperty(i, ValueProperty), v)).FirstOrDefault()
                                    : view.AsQueryable().Where($@"{ValueProperty} == @0", v).FirstOrDefault();
                        ...
                        {
                            selectedItems = values.Cast<object>().ToList();
                        }
        ...
        internal bool IsItemSelectedByValue(object v)
        {
            ...
            {
                string s => object.Equals(s, v),
                IEnumerable enumerable => enumerable.Cast<object>().Contains(v),
                null => false,
                _ => object.Equals(internalValue, v),
            };
        }
