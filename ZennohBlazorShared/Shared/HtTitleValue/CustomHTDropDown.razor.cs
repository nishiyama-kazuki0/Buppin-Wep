...
namespace ZennohBlazorShared.Shared.HtTitleValue
{
    public partial class CustomHTDropDown<TValue> : CustomHTDropDownBase<TValue>, IAsyncDisposable
    {
        ...
        private async Task OnFocusOut(Microsoft.AspNetCore.Components.Web.FocusEventArgs args)
        {
            //_ = ComService.PostLogAsync($"{UniqueID}:OnFocusOut");
            ...
            {
                //_ = ComService.PostLogAsync($"{UniqueID}:OnFocusOut ClosePopup");
                await ClosePopup();
            }
        }
        ...
        {
            //_ = ComService.PostLogAsync($"{UniqueID}:OnClick");
        ...
        {
            //_ = ComService.PostLogAsync($"{UniqueID}:OnTouchEnd");
            // ポップアップを表示する
            await OpenPopup("ArrowDown", false, true);
        }

        protected override async System.Threading.Tasks.Task<bool> ArrowDownKeyPush()
        {
            dynamic window = _js.GetWindow();
            dynamic popup = window.document.getElementById($"{PopupID}");
            string display = popup.style.display;
        ...
        protected override async Task OpenPopup(string key = "ArrowDown", bool isFilter = false, bool isFromClick = false)
        {
            ...
            //_ = ComService.PostLogAsync($"{UniqueID}:OpenPopup key:{key}");
            ...
            dynamic popup = window.document.getElementById($"{PopupID}");
            string display = popup.style.display;
            //_ = ComService.PostLogAsync($"{UniqueID}:OpenPopup display:{display}");
            if (display == "none")
            {
                FocusOutFlag = true;
