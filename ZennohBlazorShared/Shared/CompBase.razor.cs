...
namespace ZennohBlazorShared.Shared
{
    ...
    public abstract partial class CompBase : ComponentBase
    {
        ...
        protected SystemParameter _sysParam = new();
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            // セッションストレージからパラメータを取得する
            if (null != _sessionStorage)
            {
                _sysParam = await _sessionStorage.GetItemAsync<SystemParameter>(SharedConst.KEY_SYSTEM_PARAM);
            }
        }
            ...
            foreach (ComponentsInfo? c in components.Where(_ => _.ComponentName == GetType().Name + info.Property))
            {
                switch (c.ValueObjectType)
                {
                    ...
                    case (int)ComponentsInfo.EnumValueObjectType.EnumStringIndicator:
                        ...
                        object? value = typeof(CommonService).GetMethod("GetEnumValue")!.MakeGenericMethod(c.Type).Invoke(null, new object[] { strEnumStrPos });
