...
namespace ZennohBlazorShared.Shared
{
    ...
    public partial class DialogHtNotifyInfo : ChildPageBaseMobile
    {
        ...
        /// カードデータ
        /// </summary>
        protected List<DataCardMessageListInfo>? _cardMessageValuesList { get; set; } = new List<DataCardMessageListInfo>();
        ...
        private async Task ReadNotifyInfo()
        {
            try
            {
                ...
                // 一覧データ初期化
                _cardMessageValuesList = new List<DataCardMessageListInfo>();
                ...
                if (null != datas && datas.Count > 0)
                {
                    foreach (IDictionary<string, object> rows in datas)
                    {
                        ...
                        foreach (KeyValuePair<string, object> row in rows)
                        {
                            if (row.Key == "発生日時")
                            {
                                newCard.OccurredTime = ComService.GetValueString(row.Value);
                            }
                            else if (row.Key == "区分")
                            {
                                newCard.ResultCategory = ComService.GetValueInt(row.Value);
                            }
                            else if (row.Key == "メッセージ")
                            {
                                newCard.Message = ComService.GetValueString(row.Value);
                            }
