...
namespace ZennohBlazorShared.Shared
{
    public partial class DataGridExtend2 : ComponentBase
    {
        ...
        /// </summary>
        [Parameter]
        public List<ComponentColumnsInfo> Columns { get; set; } = new List<ComponentColumnsInfo>();
        /// <summary>
        /// HTフラグ
        /// </summary>
        [Parameter]
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            //セッションストレージのシステムパラメータからグリッド関連のパラメータを取得する.
            ...
            DataGridHeaderFontSize = ChildBaseService.IsHandy ? sysParams.DataGridHeaderFontSizeHT : sysParams.DataGridHeaderFontSizePC;
            DataGridColumnFontSize = ChildBaseService.IsHandy ? sysParams.DataGridColumnFontSizeHT : sysParams.DataGridColumnFontSizePC;
            DataGridCellFontSize = ChildBaseService.IsHandy ? sysParams.DataGridCellFontSizeHT : sysParams.DataGridCellFontSizePC;
            DataGridCellFontWeight = ChildBaseService.IsHandy ? sysParams.DataGridCellFontWeightHT : sysParams.DataGridCellFontWeightPC;
            DataGridFooterFontSize = ChildBaseService.IsHandy ? sysParams.DataGridFooterFontSizeHT : sysParams.DataGridFooterFontSizePC;
            GridHeight = ChildBaseService.IsHandy ? sysParams.DataGridHeightHT : sysParams.DataGridHeightPC;
        ...
        public void SelectUp()
        {
            if (SelectedData is not null && SelectedData.Count > 0)
            {
                ...
                if (idx > 0)
                {
                    SelectedData = new List<IDictionary<string, object>> { Data[idx - 1] };
                }
                else if (idx < 0)
                {
                    SelectedData = new List<IDictionary<string, object>> { Data[0] };
                }
