...
namespace ZennohBlazorShared.Shared
{
    public partial class StepItemBase : ChildPageBaseMobile
    {
        ...
        /// 倉庫ドロップダウン
        /// </summary>
        protected IList<ValueTextInfo> dropdownArea { get; set; } = new List<ValueTextInfo>();

        /// <summary>
        /// ゾーンドロップダウン
        /// </summary>
        protected IList<ValueTextInfo> dropdownZone { get; set; } = new List<ValueTextInfo>();

        /// <summary>
        /// ロケーションドロップダウン
        /// </summary>
        protected IList<ValueTextInfo> dropdownLocation { get; set; } = new List<ValueTextInfo>();

        /// <summary>
        /// 倉庫マスタ情報
        /// </summary>
        protected List<MstAreaData> _lstMstArea = new();

        /// <summary>
        /// ゾーンマスタ情報
        /// </summary>
        protected List<MstZoneData> _lstMstZone = new();

        /// <summary>
        /// ロケーションマスタ情報
        /// </summary>
        protected List<MstLocationData> _lstMstLocation = new();
        /// <summary>
        ...
        {
            return _lstMstLocation.SingleOrDefault(_ => _.LocationId == locationId)?.LocationName ?? string.Empty;
        }
        /// <summary>
        /// カードデータ
        /// </summary>
        protected List<IDictionary<string, DataCardListInfo>>? _cardValuesList { get; set; } = new List<IDictionary<string, DataCardListInfo>>();
        ...
        /// 倉庫、ゾーン、ロケーション情報を取得
        /// </summary>
        protected async Task InitComboAreaZoneLocation(bool area = true, bool zone = true, bool location = true)
        {
            List<Task> tl = new();
            if (area)
            {
        ...
        protected void SetDropdownLocation(string areaCd = "", string zoneCd = "")
        {
            ...
            List<MstLocationData> lstLocation = new();
            ...
            if (lstLocation.Count > _sysParams!.HT_LocComBoxMaxCount && ChildBaseService.IsHandy)
            {
        ...
        protected async Task ShowNotExistZone(string zoneCd)
        {
            ...
            ShowNotifyMessege(NotificationSeverity.Error, pageName, "ｿﾞｰﾝﾏｽﾀに存在しないｿﾞｰﾝCDです。");
            await Task.Delay(0);
        }
        ...
        protected async Task ShowNotExistLocation(string locationCd)
        {
            ...
            ShowNotifyMessege(NotificationSeverity.Error, pageName, "ﾛｹｰｼｮﾝﾏｽﾀに存在しないﾛｹｰｼｮﾝCDです。");
            await Task.Delay(0);
        }
        ...
        protected async Task<PalletInfo> GetPalletInfo(string strPalletNo)
        {
            try
            {
                ...
                if (null != datas && datas.Count > 0)
                {
                    ...
                    string str = ComService.GetValueString(dic, "混載");
                    if (str == "1")
                    {
                        ret.IsMixed = true;
                    }
                    str = ComService.GetValueString(dic, "引当済在庫");
                    if (str == "1")
                    {
                        ret.IsReserved = true;
                    }
                    ret.ReservedAlertTitle = ComService.GetValueString(dic, "引当済アラートタイトル");
                    ret.ReservedAlertText = ComService.GetValueString(dic, "引当済アラートテキスト");
                    ret.MixedAlertTitle = ComService.GetValueString(dic, "混載アラートタイトル");
                    ret.MixedAlertText = ComService.GetValueString(dic, "混載アラートテキスト");
                }
        ...
        protected async Task<(int, string)> GetHikiateZumiZaikoDelivery(string strPalletNo, string strDelivery)
        {
            try
            {
                ...
                {
                    IDictionary<string, object> dic = datas.First();
                    ret = ComService.GetValueInt(dic, "引当済在庫数");//記載は引当済在庫数だが実際は引当て済みの在庫が存在する指示のカウントが入る（COUNT(出荷予定集約ID)の値）
                    id = ComService.GetValueString(dic, "出荷予定集約ID");
                }
                return (ret, id);
            }
            catch (Exception ex)
        ...
        protected async Task<(int, string)> GetHikiateZumiZaiko(string strPalletNo)
        {
            try
            {
                ...
                {
                    IDictionary<string, object> dic = datas.First();
                    ret = ComService.GetValueInt(dic, "引当済在庫数");
                    id = ComService.GetValueString(dic, "出荷予定集約ID");
                }
                return (ret, id);
            }
        ...
        protected async Task LoadCardListDataInitSel(Dictionary<string, WhereParam>? where, string strViewName = "", string strInitSelectKey = "", string strInitSelectVal = "")
        {
            try
            {
                ...
                if (null != datas && datas.Count > 0)
                {
                    ...
                    {
                        Dictionary<string, DataCardListInfo> newCard = new();
                        foreach (KeyValuePair<string, object> row in rows)
                        {
                            ...
                            if (listcardHidden.ContainsKey(row.Key))
                            {
                                ...
                                {
                                    hidden = ComService.GetValueBool(obj);
                                }
                            }
                            newCard[row.Key] = new DataCardListInfo { Value = ComService.GetValueString(row.Value), Visible = !hidden };
                        }
        ...
        {
            List<IDictionary<string, DataCardListInfo>> result = new();
            try
            {
                Dictionary<string, object> listcardHidden = (Dictionary<string, object>)GetAttributes(STR_LISTCARD_HIDDEN);
                ClassNameSelect select = new()
